...
// Test G: use a separate Procurement tx (no shared context, no user) for the SELECT inside the GR hook.
Wh.prepend(function() {
    this.before('CREATE', Wh.entities.GoodsReceipts, async (req) => {
        const procureTx = Proc.tx({ /* no context, no shared cds.context */ });
        const po = await procureTx.run(
            cds.ql.SELECT.one.from(Proc.entities.PurchaseOrders).columns('ID','status')
                .where({ ID: req.data.purchaseOrder_ID ?? req.data.purchaseOrder?.ID })
        );
        await procureTx.commit();
        console.log('separate Procurement tx saw PO:', po && po.status);
    });
});

console.log('about to GR INSERT (test G)...');
const ticker = setTimeout(() => console.log('test G still pending 4s'), 4000);
try {
