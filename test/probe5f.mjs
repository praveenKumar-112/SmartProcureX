...
// Register a custom before-CREATE on GoodsReceipts that uses Procurement.tx().run (separate tx)
Wh.prepend(function() {
    this.before('CREATE', Wh.entities.GoodsReceipts, async (req) => {
        const po = await Proc.tx().run(
            cds.ql.SELECT.one
                .from(Proc.entities.PurchaseOrders)
                .columns('ID','status')
                .where({ ID: req.data.purchaseOrder_ID ?? req.data.purchaseOrder?.ID })
        );
        console.log('before-CREATE hook fired, PO via Procurement.tx:', po && po.status);
    });
});

console.log('about to GR INSERT (alt approach: Procurement.tx().run)...');
const ticker = setTimeout(() => console.log('alt still pending 4s'), 4000);
