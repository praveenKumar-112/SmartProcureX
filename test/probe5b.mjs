...
// Step: open warehouse tx, hold it open, then issue cds.db.run(SELECT PurchaseOrders) within it.
const PO = Proc.entities.PurchaseOrders;
console.log('test A: cds.db.run SELECT PurchaseOrders outside any tx');
const a = await cds.db.run(SELECT.one.from(PO).columns('ID','status').where({ID: poID}));
console.log('A ok:', a && a.ID, a && a.status);

console.log('test B: cds.db.run SELECT PurchaseOrders WITHIN Warehouse tx (no INSERT yet)');
const wtx = Wh.tx({ user });
const ticker = setTimeout(() => console.log('B still pending 3s...'), 3000);
try {
    const b = await cds.db.run(SELECT.one.from(PO).columns('ID','status').where({ID: poID}));
    clearTimeout(ticker);
    console.log('B ok:', b && b.ID, b && b.status);
} catch (e) {
    clearTimeout(ticker);
    console.log('B failed:', e?.message ?? e);
}
// do not commit wtx; just exit
process.exit(0);
