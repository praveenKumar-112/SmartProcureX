...
// Override the handler's network by YIELDING to microtask before cds.db.run:
Wh.prepend(function() {
    this.before('CREATE', Wh.entities.GoodsReceipts, async (req) => {
        // Yield to microtask queue BEFORE invoking cds.db.run, just like the wrapper.
        await Promise.resolve();
    });
});

console.log('about to GR INSERT (test H: yield before main handler...)...');
const ticker = setTimeout(() => console.log('test H still pending 4s'), 4000);
try {
